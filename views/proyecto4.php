...
<head>
    ...
    <title>CONTEO DE PERSONAJE DE PERSONALIDAD</title>
                    ...
                    fetch(`../controllers/data-2-controller.php?operacion=filtrar&editorSeleccionado=${editorSeleccionado}`)
                        ...
                        .then(alineaciones => {
                            ...
                            if (alineaciones.length > 0) {
                                ...
                                const total = alignmentCounts.reduce((acc, val) => acc + val, 0);
                                const percentages = alignmentCounts.map(count => ((count / total) * 100).toFixed(2));

                                const ctx = document.getElementById('alignmentChart').getContext('2d');
                                new Chart(ctx, {
                                    type: 'doughnut',
                                    data: {
                                        labels: alignmentLabels.map((label, index) => `${label} (${percentages[index]}%)`),
                                        datasets: [{
                                            label: 'Porcentaje de Alineaciones',
                                    ...
                                    options: {
                                        ...
                                        plugins: {
                                            ...
                                            title: {
                                                display: true,
                                                text: 'Porcentaje de Good, Bad y Neutral'
                                            }
                                        }
                                    }
                                });
